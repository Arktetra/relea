...
class ModelCheckpoint(Callback):
    ...
    def _checkpoint(self, progress: str, trainer: "relea.Trainer"):
        if not trainer.enable_checkpointing:
            return

        metric = trainer.metrics.all_metrics[self.track].compute()  # type: ignore

        if self.every_eval:
            if self.persist:
                filename = f"{type(trainer.model).__name__}-{progress}-{self.track}-{metric:.4f}.pt"
            else:
            ...
            if "loss" in self.track or self.higher_is_better:
    ...
    def after_eval(self, trainer: "relea.Trainer"):
        self._checkpoint(f"step-{trainer.step}", trainer)
